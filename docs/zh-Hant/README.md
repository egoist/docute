...
<p class="tip">
  註意這種方法智能在當前頁面使用，他並不能跳到其它的頁面的標題。跨頁面跳轉請使用 <a href="#" jump-to-id="router-link">router-link</a> 屬性。
</p>
...
首先 docute 和 gitbook 其實很類似，因為它們都是用來寫文檔並且布局很像，但是 docute 不需要妳在發布文檔前先把它編譯到 HTML 文件，並且配置很少的同時保證的實用性，妳可以用 docute 寫出很優雅的文檔。
...
不過我們也有計劃支持「編譯到 HTML 文件」這壹功能，同時也有支持[服務器端渲染的計劃](https://github.com/egoist/docute/issues/12) 的計劃，後者意味著妳不需要構建的同時也能享受更好的 SEO 體驗，雖然現在 Google 已經支持抓去動態網頁上的內容了。
