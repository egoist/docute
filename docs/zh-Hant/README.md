...
側邊欄的目錄表 (table of contents) 是從妳的 markdown 文件解析來的，我們獲取 markdown 裏的 h2 ~ h5 標題來構成這個目錄。

默認只會顯示 h2 到 h4 的標題，其它標題只有在妳頁面滾動到相應區域才會顯示，不過妳可以更改最深顯示的標題層級:
...
docute 使用 [marked](https://github.com/chjj/marked) 來解析 markdown 代碼，妳可以調整 marked 的默認參數:
