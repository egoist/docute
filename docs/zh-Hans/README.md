...
侧边栏 (或者说目录表) 是从你的 markdown 文件解析来的，我们获取 markdown 里的 h2 ~ h5 标题来构成这个目录。
...
不过我们也有计划支持「编译到 HTML 文件」这一功能，同时也有支持[服务器端渲染的计划](https://github.com/egoist/docute/issues/12)的计划，后者意味着你不需要构建的同时也能享受更好的 SEO 体验，虽然现在 Google 已经支持抓去动态网页上的内容了。
